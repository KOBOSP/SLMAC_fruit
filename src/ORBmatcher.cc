...
//            }
    ...
    int ORBmatcher::SearchReplaceKFAndMPsByProjectInGlobalMap(KeyFrame *pKF, Sophus::SE3f &Tcw, const vector<MapPoint *> &vpCandidMPs, float nThProjRad, vector<MapPoint *> &vpReplacePoint) {
        set<MapPoint *> spAlreadyFound = pKF->GetMapPoints();
        spAlreadyFound.erase(static_cast<MapPoint *>(NULL));
        int nMatches = 0, bestDist, bestIdx;
        for (int iMP = 0; iMP < vpCandidMPs.size(); iMP++) {
            MapPoint *pMP = vpCandidMPs[iMP];
            if (!pMP) {
                continue;
            }
        ...
        while (f1it != f1end && f2it != f2end) {
            ...
            if (f1it->first == f2it->first) {
                ...
                for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
                    ...
                    const bool bHaveStereoMPInKF1 = (pKF1->mvfXInRight[idx1] >= 0);
                    if (bSkipExistStereoMP)
                        if (bHaveStereoMPInKF1)
                            continue;
                    ...
                    for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++) {
                        ...
                        if (vMatches12[idx2] > -1 || pMP2)
                            continue;
                        const bool bHaveStereoMPInKF2 = (pKF2->mvfXInRight[idx2] >= 0);
                        if (bSkipExistStereoMP)
                            if (bHaveStereoMPInKF2)
                                continue;
                        ...
                        if (!bHaveStereoMPInKF1 && !bHaveStereoMPInKF2) {
    ...
    int ORBmatcher::SearchFrameAndFrameByProject(Frame &CurrentFrame, const Frame &LastFrame, const float nThProjRad,
        ...
        for (int i = 0; i < LastFrame.mnKPsLeftNum; i++) {
            ...
            for (vector<size_t>::const_iterator vit = vIndices2.begin(), vend = vIndices2.end();
                ...
                const cv::Mat &dCKF = CurrentFrame.mDescriptorsLeft.row(i2);
                const int Dist = GetDescriptorDistance(dMP, dCKF);
                if (Dist < bestDist) {
                    bestDist = Dist;
                    bestIdx2 = i2;
                }
    ...
    int ORBmatcher::SearchFrameAndKFByProject(Frame &CurrentFrame, KeyFrame *pKF, const set<MapPoint *> &sAlreadyFound,
        ...
        for (size_t i = 0, iend = vpMPs.size(); i < iend; i++) {
            ...
            for (vector<size_t>::const_iterator vit = vIndices2.begin(); vit != vIndices2.end(); vit++) {
                ...
                if (CurrentFrame.mvpMPs[i2])
                    continue;
                const cv::Mat &dInKF = CurrentFrame.mDescriptorsLeft.row(i2);
                const int Dist = GetDescriptorDistance(dMP, dInKF);
                if (Dist < bestDist) {
                    bestDist = Dist;
                    bestIdx2 = i2;
                }
