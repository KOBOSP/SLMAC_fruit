...
namespace ORB_SLAM3 {
    ...
    void Optimizer::GlobalBundleAdjustemntWithoutImu(Map *pMap, int nIterations, bool *pbStopFlag,
        ...
        vector<ORB_SLAM3::EdgeMonoPoseAndMPNoImu *> vpEdgesMono;
        ...
        for (size_t i = 0; i < vpMP.size(); i++) {
            ...
            for (map<KeyFrame *, tuple<int, int>>::const_iterator mit = observations.begin();
                ...
                if (leftIndex != -1 && pKF->mvfXInRight[get<0>(mit->second)] < 0) {
                    const cv::KeyPoint &kpUn = pKF->mvKPsUn[leftIndex];

                    Eigen::Matrix<double, 2, 1> obs;
                    obs << kpUn.pt.x, kpUn.pt.y;

                    ORB_SLAM3::EdgeMonoPoseAndMPNoImu *e = new ORB_SLAM3::EdgeMonoPoseAndMPNoImu();

                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(id)));
                    e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(pKF->mnId)));
                    e->setMeasurement(obs);
                    const float &invSigma2 = pKF->mvfInvLevelSigma2[kpUn.octave];
                ...
                } else if (leftIndex != -1 && pKF->mvfXInRight[leftIndex] >= 0) //Stereo observation
                {
                    ...
                    const float kp_ur = pKF->mvfXInRight[get<0>(mit->second)];
        ...
        for (size_t i = 0; i < vpKFs.size(); i++) {
            ...
            if (nLoopKF == pMap->GetOriginKF()->mnId) {
                ...
                pKF->mLMGBATcw = Sophus::SE3d(SE3quat.rotation(), SE3quat.translation()).cast<float>();
                pKF->mnLMGBAFlag = nLoopKF;

                Sophus::SE3f mTwc = pKF->GetPoseInverse();
                Sophus::SE3f mTcGBA_c = pKF->mLMGBATcw * mTwc;
                ...
                if (dist > 1) {
                    ...
                    for (size_t i2 = 0, iend = vpEdgesMono.size(); i2 < iend; i2++) {
                        ORB_SLAM3::EdgeMonoPoseAndMPNoImu *e = vpEdgesMono[i2];
        ...
        for (size_t i = 0; i < vpMP.size(); i++) {
            ...
            if (nLoopKF == pMap->GetOriginKF()->mnId) {
                ...
                pMP->mLMGBAPos = vPoint->estimate().cast<float>();
                pMP->mnLMGBAFlag = nLoopKF;
            }
        }
    }

    void
    Optimizer::GlobalBundleAdjustemetWithImu(Map *pMap, int its, const bool bFixLocal, const long unsigned int nLoopKFId,
                                             bool *pbStopFlag, bool bInited, float GInfo, float AInfo,
        ...
        for (size_t i = 0; i < vpKFs.size(); i++) {
            ...
            VP->setId(pKFi->mnId);
            VP->setFixed(pKFi->mnId == pMap->GetInitKFId());
            optimizer.addVertex(VP);
            pIncKF = pKFi;

            //KFPose6DoF=pKFi->mnId
            //KFVelocity=maxKFid + 3 * (pKFi->mnId) + 1
            //KFGyrBias=maxKFid + 3 * (pKFi->mnId) + 2 !bInited
            //KFAccBias=maxKFid + 3 * (pKFi->mnId) + 3 !bInited
            //KFGyrBias=4 * maxKFid + 2 bInited
            //KFAccBias=4 * maxKFid + 3 bInited
            //MPointXYZ=pMP->mnId + maxKFid * 6 + 1;
        ...
        for (size_t i = 0; i < vpKFs.size(); i++) {
            KeyFrame *pKFi = vpKFs[i];
            if (!pKFi->mPrevKF || !pKFi) {
                continue;
            }
            if (pKFi->mPrevKF->isBad() || pKFi->isBad()) {
                continue;
            }
            if (pKFi->mPrevKF->mnId > maxKFid && pKFi->mnId > maxKFid) {
                continue;
            }
            ...
            if (pKFi->bImu && pKFi->mPrevKF->bImu) {
                ...
                EdgeImuRVPOnlyInImu *ei = new EdgeImuRVPOnlyInImu(pKFi->mpImuPreintegrated);
                ...
                if (!bInited) {
                    EdgeTwoGyrBiasInImu *egr = new EdgeTwoGyrBiasInImu();
                    egr->setVertex(0, VG1);
                    egr->setVertex(1, VG2);
                    ...
                    EdgeTwoAccBiasInImu *ear = new EdgeTwoAccBiasInImu();
                    ear->setVertex(0, VA1);
                    ear->setVertex(1, VA2);
        ...
        if (bInited) {
            ...
            Eigen::Vector3f bGAPrior;
            bGAPrior.setZero();

            EdgePriorAccBiasInImu *epa = new EdgePriorAccBiasInImu(bGAPrior);
            epa->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VA));
            epa->setInformation(AInfo * Eigen::Matrix3d::Identity());
            optimizer.addEdge(epa);

            EdgePriorGyrBiasInImu *epg = new EdgePriorGyrBiasInImu(bGAPrior);
            epg->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VG));
            epg->setInformation(GInfo * Eigen::Matrix3d::Identity());
            optimizer.addEdge(epg);
        }

        //visual link
        const float fThHuberMono = sqrt(5.991);
        const float fThHuberStereo = sqrt(7.815);
        const unsigned long nMPIdBegin = maxKFid * 6 + 5;
        vector<bool> vbNotIncludedMP(vpMPs.size(), false);
        for (size_t i = 0; i < vpMPs.size(); i++) {
            MapPoint *pMP = vpMPs[i];
            g2o::VertexSBAPointXYZ *vPoint = new g2o::VertexSBAPointXYZ();
            vPoint->setEstimate(pMP->GetWorldPos().cast<double>());
            unsigned long id = pMP->mnId + nMPIdBegin + 1;
            vPoint->setId(id);
            vPoint->setMarginalized(true);
            optimizer.addVertex(vPoint);

            const map<KeyFrame *, tuple<int, int>> observations = pMP->GetObsKFAndLRIdx();
            bool bMPIsFixed = true;
            for (map<KeyFrame *, tuple<int, int>>::const_iterator mit = observations.begin(), mend = observations.end();
                ...
                const int leftIndex = get<0>(mit->second);
                cv::KeyPoint kpUn;
                if (leftIndex != -1 && pKFi->mvfXInRight[get<0>(mit->second)] < 0){
                    ...
                    g2o::RobustKernelHuber *rk = new g2o::RobustKernelHuber;
                    e->setRobustKernel(rk);
                    rk->setDelta(fThHuberMono);
                    optimizer.addEdge(e);
                } else if (leftIndex != -1 && pKFi->mvfXInRight[leftIndex] >= 0){
                    kpUn = pKFi->mvKPsUn[leftIndex];
        ...
        for (size_t i = 0; i < vpKFs.size(); i++) {
            ...
            if (nLoopKFId == 0) {
                ...
                pKFi->mLMGBATcw = Sophus::SE3f(VP->estimate().Rcw[0].cast<float>(), VP->estimate().tcw[0].cast<float>());
                pKFi->mnLMGBAFlag = nLoopKFId;
            }
            if (pKFi->bImu) {
                ...
                if (nLoopKFId == 0) {
                    ...
                    pKFi->mLMGBAVwb = VV->estimate().cast<float>();
                }
    ...
    int Optimizer::PoseInertialOptimizationLastKeyFrame(Frame *pFrame, bool bRecInit) {
        ...
        vector<EdgeMonoPoseOnlyInImu *> vpEdgesMono;
        vector<EdgeStereoPoseOnlyInImu *> vpEdgesStereo;
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (pMP) {
                    ...
                    if (pFrame->mvfXInRight[i] < 0) {
                        ...
                        EdgeMonoPoseOnlyInImu *e = new EdgeMonoPoseOnlyInImu(pMP->GetWorldPos(), 0);
                        ...
                        EdgeStereoPoseOnlyInImu *e = new EdgeStereoPoseOnlyInImu(pMP->GetWorldPos());
        ...
        EdgeImuRVPOnlyInImu *ei = new EdgeImuRVPOnlyInImu(pFrame->mpImuFromPrevKF);
        ...
        for (size_t it = 0; it < 4; it++) {
            ...
            for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
                EdgeMonoPoseOnlyInImu *e = vpEdgesMono[i];
            ...
            for (size_t i = 0, iend = vpEdgesStereo.size(); i < iend; i++) {
                EdgeStereoPoseOnlyInImu *e = vpEdgesStereo[i];
        ...
        if ((nInliers < 30) && !bRecInit) {
            ...
            EdgeMonoPoseOnlyInImu *e1;
            EdgeStereoPoseOnlyInImu *e2;
    ...
    int Optimizer::PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit) {
        ...
        EdgeImuRVPOnlyInImu *ei = new EdgeImuRVPOnlyInImu(pFrame->mpImuFromPrevFrame);
        ...
        EdgePriorRVPBiasGAInImu *ep = new EdgePriorRVPBiasGAInImu(pFp->mpcpi);
    ...
    void Optimizer::LocalBAWithoutImu(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int &nFixedKFsNum, int &nOptKFsNum,
        ...
        for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end();
            ...
            vector<MapPoint *> vpMPs = pKFi->GetVectorMapPointsInKF();
        ...
        nFixedKFsNum = lFixedKFs.size() + nFixedKFsNum;
        ...
        nOptKFsNum = lLocalKeyFrames.size();
        ...
        for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end();
            ...
            for (map<KeyFrame *, tuple<int, int>>::const_iterator mit = observations.begin(), mend = observations.end();
                ...
                if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap) {
                    ...
                    if (leftIndex != -1 && pKFi->mvfXInRight[get<0>(mit->second)] < 0) {
                        const cv::KeyPoint &kpUn = pKFi->mvKPsUn[leftIndex];
                        Eigen::Matrix<double, 2, 1> obs;
                        obs << kpUn.pt.x, kpUn.pt.y;
        ...
        for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
            ORB_SLAM3::EdgeMonoPoseAndMPNoImu *e = vpEdgesMono[i];
    ...
    void Optimizer::LocalBAWithImu(KeyFrame *pCurKF, bool *pbStopFlag, Map *pMap, int &num_fixedKF, int &num_OptKF,
                                   int &num_MPs, int &num_edges, bool bLarge, bool bRecInit) {
        Map *pCurrentMap = pCurKF->GetMap();
        const int nMaxOptVisKFs = 100;
        const int nMaxFixKFs = 100;
        int maxOpt = 10;
        int nOptIterTimes = 10;
        if (bLarge) {
            maxOpt = 25;
            nOptIterTimes = 4;
        }
        const int nMaxOptImuKFs = std::min((int) pCurrentMap->GetKeyFramesNumInMap() - 2, maxOpt);
        const unsigned long nCurKFId = pCurKF->mnId;
        vector<KeyFrame *> vpOptImuKFs;
        ...
        int nOptImuKFsNum = vpOptImuKFs.size();
        // Optimizable points seen by temporal optimizable keyframes
        list<MapPoint *> lLocalMapPoints;
        for (int i = 0; i < nOptImuKFsNum; i++) {
            vector<MapPoint *> vpMPs = vpOptImuKFs[i]->GetVectorMapPointsInKF();
            for (vector<MapPoint *>::iterator vit = vpMPs.begin(), vend = vpMPs.end(); vit != vend; vit++) {
                MapPoint *pMP = *vit;
                if (!pMP) {
                    continue;
        ...
        const vector<KeyFrame *> vpNeighsKFs = pCurKF->GetVectorCovisibleKeyFrames();
        for (int i = 0, iend = vpNeighsKFs.size(); i < iend; i++) {
            if (lpOptVisKFs.size() >= nMaxOptVisKFs)
                break;
        ...
        for (int i = 0; i < nOptImuKFsNum; i++) {
            ...
            if (pKFi->bImu) {
                ...
                optimizer.addVertex(VG);
                VertexAccBias *VA = new VertexAccBias(pKFi);
                VA->setId(nCurKFId + 3 * (pKFi->mnId) + 3);
                VA->setFixed(false);
                optimizer.addVertex(VA);
                pCurrentMap->msOptImuKFs.insert(pKFi->mnId);
            }
        }
        ...
        for (int i = 0; i < nOptImuKFsNum; i++) {
            ...
            if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated) {
                pKFi->mpImuPreintegrated->SetNewBias(pKFi->mPrevKF->GetImuBias());
                g2o::HyperGraph::Vertex *VP1 = optimizer.vertex(pKFi->mPrevKF->mnId);
                g2o::HyperGraph::Vertex *VV1 = optimizer.vertex(nCurKFId + 3 * (pKFi->mPrevKF->mnId) + 1);
                g2o::HyperGraph::Vertex *VG1 = optimizer.vertex(nCurKFId + 3 * (pKFi->mPrevKF->mnId) + 2);
                g2o::HyperGraph::Vertex *VA1 = optimizer.vertex(nCurKFId + 3 * (pKFi->mPrevKF->mnId) + 3);
                g2o::HyperGraph::Vertex *VP2 = optimizer.vertex(pKFi->mnId);
                g2o::HyperGraph::Vertex *VV2 = optimizer.vertex(nCurKFId + 3 * (pKFi->mnId) + 1);
                g2o::HyperGraph::Vertex *VG2 = optimizer.vertex(nCurKFId + 3 * (pKFi->mnId) + 2);
                g2o::HyperGraph::Vertex *VA2 = optimizer.vertex(nCurKFId + 3 * (pKFi->mnId) + 3);
                ...
                vEdgeImuVisRVP[i] = new EdgeImuRVPOnlyInImu(pKFi->mpImuPreintegrated);
                vEdgeImuVisRVP[i]->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VP1));
                vEdgeImuVisRVP[i]->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VV1));
                vEdgeImuVisRVP[i]->setVertex(2, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VG1));
                vEdgeImuVisRVP[i]->setVertex(3, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VA1));
                vEdgeImuVisRVP[i]->setVertex(4, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VP2));
                vEdgeImuVisRVP[i]->setVertex(5, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VV2));

                if (i == nOptImuKFsNum - 1 || bRecInit) {
                    ...
                    g2o::RobustKernelHuber *rki = new g2o::RobustKernelHuber;
                    vEdgeImuVisRVP[i]->setRobustKernel(rki);
                    if (i == nOptImuKFsNum - 1)
                        vEdgeImuVisRVP[i]->setInformation(vEdgeImuVisRVP[i]->information() * 1e-2);
                    rki->setDelta(sqrt(16.92));
                }
                optimizer.addEdge(vEdgeImuVisRVP[i]);

                vEdgeGyrBias[i] = new EdgeTwoGyrBiasInImu();
                vEdgeGyrBias[i]->setVertex(0, VG1);
                vEdgeGyrBias[i]->setVertex(1, VG2);
                Eigen::Matrix3d InfoG = pKFi->mpImuPreintegrated->C.block<3, 3>(9, 9).cast<double>().inverse();
                vEdgeGyrBias[i]->setInformation(InfoG);
                optimizer.addEdge(vEdgeGyrBias[i]);

                vEdgeAccBias[i] = new EdgeTwoAccBiasInImu();
                vEdgeAccBias[i]->setVertex(0, VA1);
                vEdgeAccBias[i]->setVertex(1, VA2);
                Eigen::Matrix3d InfoA = pKFi->mpImuPreintegrated->C.block<3, 3>(12, 12).cast<double>().inverse();
            ...
            if (pMap->GetRtkInitialized()) {
                EdgePriorKFRtk *EdgeKFRtkToLocal = new EdgePriorKFRtk(
                        (s12i * R12i * pKFi->GetRtkTransF() + t12i).cast<double>());
        ...
        // Mono
        vector<EdgeMonoPoseAndMPInImu *> vpEdgesMono;
        vpEdgesMono.reserve(nExpectedSize);
        ...
        // Stereo
        vector<EdgeStereoPoseAndMPInImu *> vpEdgesStereo;
        vpEdgesStereo.reserve(nExpectedSize);
        ...
        const unsigned long nMPIdBegin = nCurKFId * 5;
        ...
        for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end();
            ...
            for (map<KeyFrame *, tuple<int, int>>::const_iterator mit = observations.begin(), mend = observations.end();
                ...
                if (leftIndex != -1 && pKFi->mvfXInRight[leftIndex] < 0) {
                    ...
                    vpEdgeKFMono.emplace_back(pKFi);
                    vpMapPointEdgeMono.emplace_back(pMP);
                } else if (leftIndex != -1 && pKFi->mvfXInRight[leftIndex] > 0)// Stereo observation
                {
                    kpUn = pKFi->mvKPsUn[leftIndex];
                    const float kp_ur = pKFi->mvfXInRight[leftIndex];
                    Eigen::Matrix<double, 3, 1> obs;
        ...
        float fErrBegin = optimizer.activeRobustChi2();
        optimizer.optimize(nOptIterTimes); // Originally to 2
        float fErrEnd = optimizer.activeRobustChi2();
        if (pbStopFlag)
            optimizer.setForceStopFlag(pbStopFlag);
        ...
        // Mono
        for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
            EdgeMonoPoseAndMPInImu *e = vpEdgesMono[i];
            MapPoint *pMP = vpMapPointEdgeMono[i];
        ...
        // Stereo
        for (size_t i = 0, iend = vpEdgesStereo.size(); i < iend; i++) {
            EdgeStereoPoseAndMPInImu *e = vpEdgesStereo[i];
            MapPoint *pMP = vpMapPointEdgeStereo[i];
        ...
        if ((2 * fErrBegin < fErrEnd || isnan(fErrBegin) || isnan(fErrEnd)) && !bLarge) //bGN)
        {
    ...
    void Optimizer::OptimizeEssentialGraph(KeyFrame *pCurKF, vector<KeyFrame *> &vpFixedKFs,
        ...
        Verbose::PrintMess("Opt_Essential: There are " + to_string(vpNonCorrectedMPs.size()) +
                           " MPs non-corrected in the merged map", Verbose::VERBOSITY_DEBUG);
    ...
    Optimizer::OptimizeKFsSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpKF1IdxToKF2MP, g2o::Sim3 &g2oS12,
        ...
        for (int i = 0; i < nMatchesNum; i++) {
            ...
            if (nMP2IdxInKF2 < 0 && !bAllPoints) {
                Verbose::PrintMess(
                        "    Remove point -> nMP2IdxInKF2: " + to_string(nMP2IdxInKF2) + "; bAllPoints: " +
                        to_string(bAllPoints),
                        Verbose::VERBOSITY_DEBUG);
                continue;
            }
        ...
        for (size_t i = 0; i < vpEdges12.size(); i++) {
            ...
            ORB_SLAM3::EdgeSim3Cam2MonoPoseAndMPNoImu *e21 = vpEdges21[i];
            if (!e12 || !e21)
                continue;
    ...
    void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &fScale, Eigen::Vector3d &bg,
                                         Eigen::Vector3d &ba, bool bMono, Eigen::MatrixXd &ImuInfo, bool bFixedVel,
                                         bool bGauss, float GInfo, float AInfo) {
        ...
        bprior.setZero();
        EdgePriorAccBiasInImu *epa = new EdgePriorAccBiasInImu(bprior);
        epa->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VA));
        epa->setInformation(AInfo * Eigen::Matrix3d::Identity());
        optimizer.addEdge(epa);

        // prior gyr bias
        EdgePriorGyrBiasInImu *epg = new EdgePriorGyrBiasInImu(bprior);
        epg->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VG));
        epg->setInformation(GInfo * Eigen::Matrix3d::Identity());
        optimizer.addEdge(epg);
        ...
        VertexScale *VS = new VertexScale(fScale);
        VS->setId(maxKFid * 2 + 5);
        VS->setFixed(!bMono); // Fixed for stereo case
        optimizer.addVertex(VS);

        // Graph edges, IMU links with gravity and fScale
        vector<EdgeImuRVPGSInImu *> vpei;
        ...
        for (size_t i = 0; i < vpKFs.size(); i++) {
            ...
            if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
                ...
                EdgeImuRVPGSInImu *ei = new EdgeImuRVPGSInImu(pKFi->mpImuPreintegrated);
                ei->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VP1));
                ei->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VV1));
        ...
        bg << VG->estimate();
        ba << VA->estimate();
        IMU::Bias biasAG(ba[0], ba[1], ba[2], bg[0], bg[1], bg[2]);
    ...
    void
    Optimizer::InertialOptimization(Map *pMap, Eigen::Vector3d &bg, Eigen::Vector3d &ba, float priorG, float priorA) {
    ...
    void
    Optimizer::LocalBundleAdjustment(KeyFrame *pMainKF, vector<KeyFrame *> vpAdjustKF, vector<KeyFrame *> vpFixedKF,
                                     bool *pbStopFlag) {
        ...
        for (unsigned int i = 0; i < vpMPs.size(); ++i) {
            ...
            for (map<KeyFrame *, tuple<int, int>>::const_iterator mit = observations.begin();
                ...
                if (pKF->isBad() || pKF->mnId > maxKFid || pKF->mnBALocalForMerge != pMainKF->mnId ||
                    !pKF->GetIdxMapPoint(get<0>(mit->second)))
                    continue;

                nEdges++;

                const cv::KeyPoint &kpUn = pKF->mvKPsUn[get<0>(mit->second)];

                if (pKF->mvfXInRight[get<0>(mit->second)] < 0) //Monocular
                {
                    mpObsMPs[pMPi]++;
                    Eigen::Matrix<double, 2, 1> obs;
                    obs << kpUn.pt.x, kpUn.pt.y;
        ...
        if (bDoMore) {
            ...
            Verbose::PrintMess(
                    "[BA]: First optimization(Huber), there are " + to_string(badMonoMP) + " monocular and " +
                    to_string(badStereoMP) + " stereo bad edges", Verbose::VERBOSITY_DEBUG);
        ...
        Verbose::PrintMess("[BA]: Second optimization, there are " + to_string(badMonoMP) + " monocular and " +
                           to_string(badStereoMP) + " sterero bad edges", Verbose::VERBOSITY_DEBUG);
        ...
        for (unsigned int i = 0; i < vpMPs.size(); ++i) {
            ...
            for (map<KeyFrame *, tuple<int, int>>::const_iterator mit = observations.begin();
                 mit != observations.end(); mit++) {
                KeyFrame *pKF = mit->first;
                if (pKF->isBad() || pKF->mnId > maxKFid || pKF->mnBAOptFlagInLM != pMainKF->mnId ||
                    !pKF->GetIdxMapPoint(get<0>(mit->second)))
                    continue;
    ...
    void Optimizer::MergeInertialBA(KeyFrame *pCurrKF, KeyFrame *pMergeKF, bool *pbStopFlag, Map *pMap,
        ...
        for (int i = 0; i < N; i++) {
            vector<MapPoint *> vpMPs = vpOptimizableKFs[i]->GetVectorMapPointsInKF();
            for (vector<MapPoint *>::iterator vit = vpMPs.begin(), vend = vpMPs.end(); vit != vend; vit++) {
                // Using mnBAOptFlagInLM we avoid redundance here, one MP can not be added several times to lLocalMapPoints
                MapPoint *pMP = *vit;
                if (pMP)
                    if (!pMP->isBad())
        ...
        for (vector<pair<MapPoint *, int>>::iterator lit = pairs.begin(), lend = pairs.end(); lit != lend; lit++, i++) {
            ...
            for (map<KeyFrame *, tuple<int, int>>::iterator mit = observations.begin(), mend = observations.end();
                ...
                if (pKFi->mnBAOptFlagInLM != pCurrKF->mnId &&
                    pKFi->mnBAFixFlagInLM != pCurrKF->mnId) // If optimizable or already included...
                {
        ...
        // Create intertial constraints
        vector<EdgeImuRVPOnlyInImu *> vei(N, (EdgeImuRVPOnlyInImu *) NULL);
        vector<EdgeTwoGyrBiasInImu *> vegr(N, (EdgeTwoGyrBiasInImu *) NULL);
        vector<EdgeTwoAccBiasInImu *> vear(N, (EdgeTwoAccBiasInImu *) NULL);
        for (int i = 0; i < N; i++) {
            ...
            if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated) {
                ...
                vei[i] = new EdgeImuRVPOnlyInImu(pKFi->mpImuPreintegrated);
                ...
                vegr[i] = new EdgeTwoGyrBiasInImu();
                ...
                vear[i] = new EdgeTwoAccBiasInImu();
        ...
        const unsigned long nMPIdBegin = maxKFid * 5;
        ...
        for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end();
            ...
            for (map<KeyFrame *, tuple<int, int>>::const_iterator mit = observations.begin(), mend = observations.end();
                ...
                if ((pKFi->mnBAOptFlagInLM != pCurrKF->mnId) && (pKFi->mnBAFixFlagInLM != pCurrKF->mnId))
                    continue;
                ...
                if (!pKFi->isBad()) {
                    const cv::KeyPoint &kpUn = pKFi->mvKPsUn[get<0>(mit->second)];

                    if (pKFi->mvfXInRight[get<0>(mit->second)] < 0) // Monocular observation
                    {
                        Eigen::Matrix<double, 2, 1> obs;
                        obs << kpUn.pt.x, kpUn.pt.y;

                        EdgeMonoPoseAndMPInImu *e = new EdgeMonoPoseAndMPInImu();
                        e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(id)));
                        e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(pKFi->mnId)));
                        e->setMeasurement(obs);
                        const float &invSigma2 = pKFi->mvfInvLevelSigma2[kpUn.octave];
                    ...
                    } else // stereo observation
                    {
                        ...
                        EdgeStereoPoseAndMPInImu *e = new EdgeStereoPoseAndMPInImu();
    ...
    void Optimizer::OptimizeEssentialGraph4DoF(Map *pMap, KeyFrame *pLoopKF, KeyFrame *pCurKF,
        ...
        for (map<KeyFrame *, set<KeyFrame *> >::const_iterator mit = LoopConnections.begin(), mend = LoopConnections.end();
            ...
            for (set<KeyFrame *>::const_iterator sit = spConnections.begin(), send = spConnections.end();
                ...
                EdgeTwoPoseRTInImu *e = new EdgeTwoPoseRTInImu(Tij);
                e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(nIDj)));
                e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(nIDi)));
        ...
        for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
            ...
            for (set<KeyFrame *>::const_iterator sit = sLoopEdges.begin(), send = sLoopEdges.end();
                ...
                if (pLKF->mnId < pKF->mnId) {
                    ...
                    EdgeTwoPoseRTInImu *e = new EdgeTwoPoseRTInImu(Til);
                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(nIDi)));
            ...
            for (vector<KeyFrame *>::const_iterator vit = vpConnectedKFs.begin(); vit != vpConnectedKFs.end(); vit++) {
                ...
                if (pKFn && pKFn != pParentKF && pKFn != prevKF && pKFn != pKF->mNextKF && !pKF->hasChild(pKFn) &&
                    ...
                    if (!pKFn->isBad() && pKFn->mnId < pKF->mnId) {
                        ...
                        Tin(3, 3) = 1.;
                        EdgeTwoPoseRTInImu *e = new EdgeTwoPoseRTInImu(Tin);
                        e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(nIDi)));
