 ...
 * @return 像素坐标
 */
    cv::Point2f KannalaBrandt8::ProjectMPToKP(const cv::Point3f &p3D) {
 ...
 * @return 像素坐标
 */
    Eigen::Vector2d KannalaBrandt8::ProjectMPToKP(const Eigen::Vector3d &v3D) {
 ...
 * @return 像素坐标
 */
    Eigen::Vector2f KannalaBrandt8::ProjectMPToKP(const Eigen::Vector3f &v3D) {
        ...
        /*cv::Point2f cvres = this->ProjectMPToKP(cv::Point3f(v3D[0],v3D[1],v3D[2]));
 ...
 */
    Eigen::Vector3f KannalaBrandt8::UnprojectEig(const cv::Point2f &p2D) {
        cv::Point3f ray = this->UnprojectCv(p2D);
 ...
 * @return 
 */
    cv::Point3f KannalaBrandt8::UnprojectCv(const cv::Point2f &p2D) {
 ...
 */
    Eigen::Matrix<double, 2, 3> KannalaBrandt8::ProjectJac(const Eigen::Vector3d &v3D) {
        ...
        JacGood(1, 0) =
                mvParameters[1] *
                (fd * v3D[2] * v3D[1] * v3D[0] / (r2 * (r2 + z2)) - f * v3D[1] * v3D[0] / r3);  // ∂v/∂x

        JacGood(0, 1) =
                mvParameters[0] *
                (fd * v3D[2] * v3D[1] * v3D[0] / (r2 * (r2 + z2)) - f * v3D[1] * v3D[0] / r3);  // ∂u/∂y
 ...
 */
    ...
    Eigen::Matrix3f KannalaBrandt8::GetKEig() {
        Eigen::Matrix3f K;
 ...
 */
    float KannalaBrandt8::TriangulateMatches(
        ...
        // 5. 做下投影计算重投影误差
        Eigen::Vector2f uv1 = this->ProjectMPToKP(x3D);
        ...
        Eigen::Vector2f uv2 = pCamera2->ProjectMPToKP(x3D2);
 ...
 */
    ...
    bool KannalaBrandt8::IsEqual(GeometricCamera *pCam) {
        ...
        if (ParameterSize() != pKBCam->ParameterSize())
            return false;
