 ...
 * @brief 单目投影
 */
    Eigen::Vector2d ImuCamPose::ProjectMono(const Eigen::Vector3d &Xw, int cam_idx) const {
        Eigen::Vector3d Xc = Rcw[cam_idx] * Xw + tcw[cam_idx];
        return pCamera[cam_idx]->ProjectMPToKP(Xc);
    }
 ...
 */
    Eigen::Vector3d ImuCamPose::ProjectStereo(const Eigen::Vector3d &Xw, int cam_idx) const {
        ...
        Eigen::Vector3d pc;
        double invZ = 1 / Pc(2);
        pc.head(2) = pCamera[cam_idx]->ProjectMPToKP(Pc);
        pc(2) = pc(0) - bf * invZ;
        return pc;
    }
 ...
 */
    ...
    VertexGyrBias::VertexGyrBias(KeyFrame *pKF) {
        setEstimate(pKF->GetGyroBias().cast<double>());
    }

    VertexGyrBias::VertexGyrBias(Frame *pF) {
        Eigen::Vector3d bg;
...
// 计算雅克比矩阵
    void EdgeImuRVPOnlyInImu::linearizeOplus() {
        ...
        const VertexPose6DoF *VP2 = static_cast<const VertexPose6DoF *>(_vertices[4]);
...
// 计算误差
    void EdgeImuRVPGSInImu::computeError() {
        ...
        const VertexPose6DoF *VP2 = static_cast<const VertexPose6DoF *>(_vertices[4]);
        const VertexVelocity *VV2 = static_cast<const VertexVelocity *>(_vertices[5]);
        const VertexGraDir *VGDir = static_cast<const VertexGraDir *>(_vertices[6]);
