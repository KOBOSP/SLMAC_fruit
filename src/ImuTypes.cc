 ...
 */
        IntegratedRotation::IntegratedRotation(const Eigen::Vector3f &angVel, const Bias &imuBias, const float &time) {
            // 得到考虑偏置后的角度旋转
            const float x = (angVel(0) - imuBias.mBGX) * time;
            const float y = (angVel(1) - imuBias.mBGY) * time;
            const float z = (angVel(2) - imuBias.mBGZ) * time;
 ...
 */
        Preintegrated::Preintegrated(const Bias &InputBias, const Calib &calib) {
            mCovNoise = calib.CovNoise;
            mCovWalk = calib.CovWalk;
            Initialize(InputBias);
        }

// Copy constructor
        Preintegrated::Preintegrated(Preintegrated *pImuPre)
                : mfTs(pImuPre->mfTs), C(pImuPre->C),
                  mCovNoise(pImuPre->mCovNoise), mCovWalk(pImuPre->mCovWalk), mBiasOri(pImuPre->mBiasOri), dR(pImuPre->dR), dV(pImuPre->dV),
                  dP(pImuPre->dP), JRg(pImuPre->JRg), JVg(pImuPre->JVg), JVa(pImuPre->JVa), JPg(pImuPre->JPg),
                  JPa(pImuPre->JPa),
                  mAvgAcc(pImuPre->mAvgAcc), mAvgGyr(pImuPre->mAvgGyr), mBiasUpdate(pImuPre->mBiasUpdate),
                  mBiasDelta(pImuPre->mBiasDelta), mvMeasurements(pImuPre->mvMeasurements) {
        }
 ...
 */
        void Preintegrated::CopyFrom(Preintegrated *pImuPre) {
            mfTs = pImuPre->mfTs;
            C = pImuPre->C;
            mCovNoise = pImuPre->mCovNoise;
            mCovWalk = pImuPre->mCovWalk;
            mBiasOri.CopyFrom(pImuPre->mBiasOri);
            dR = pImuPre->dR;
            dV = pImuPre->dV;
            dP = pImuPre->dP;
            ...
            JPg = pImuPre->JPg;
            JPa = pImuPre->JPa;
            mAvgAcc = pImuPre->mAvgAcc;
            mAvgGyr = pImuPre->mAvgGyr;
            mBiasUpdate.CopyFrom(pImuPre->mBiasUpdate);
            mBiasDelta = pImuPre->mBiasDelta;
 ...
 * @param InputBias 偏置
 */
        void Preintegrated::Initialize(const Bias &InputBias) {
            dR.setIdentity();
            dV.setZero();
            dP.setZero();
            JRg.setZero();
            ...
            mBiasDelta.setZero();
            mBiasOri = InputBias;
            mBiasUpdate = InputBias;  // 更新后的偏置
            mAvgAcc.setZero();  // 平均加速度
            mAvgGyr.setZero();  // 平均角速度
            mfTs = 0.0f;
 ...
 */
        void Preintegrated::Reintegrate() {
            ...
            Initialize(mBiasUpdate);
            for (size_t i = 0; i < aux.size(); i++)
                IntegrateNewMeasurement(aux[i].mAcc, aux[i].mGyr, aux[i].mTs);
        }
 ...
 */
        void Preintegrated::IntegrateNewMeasurement(const Eigen::Vector3f &Acc, const Eigen::Vector3f &Gyr,
            ...
            // 考虑偏置后的加速度、角速度
            Eigen::Vector3f AccDeBias, GyrDeBias;
            AccDeBias << Acc(0) - mBiasOri.mBAX, Acc(1) - mBiasOri.mBAY, Acc(2) - mBiasOri.mBAZ;
            GyrDeBias << Gyr(0) - mBiasOri.mBGX, Gyr(1) - mBiasOri.mBGY, Gyr(2) - mBiasOri.mBGZ;
            ...
            Eigen::Matrix<float, 3, 3> Wacc = Sophus::SO3f::hat(AccDeBias);

            A.block<3, 3>(3, 0) = -dR * TDelta * Wacc;
            A.block<3, 3>(6, 0) = -0.5f * dR * TDelta * TDelta * Wacc;
            A.block<3, 3>(6, 3) = Eigen::DiagonalMatrix<float, 3>(TDelta, TDelta, TDelta);
            ...
            JPg = JPg + JVg * TDelta - 0.5f * dR * TDelta * TDelta * Wacc * JRg;
            JVa = JVa - dR * TDelta;
            ...
            JRg = dRi.deltaR.transpose() * JRg - dRi.rightJ * TDelta;
 ...
 */
        void Preintegrated::MergePrevious(Preintegrated *pPrev) {
            ...
            Bias bav;
            bav.mBGX = mBiasUpdate.mBGX;
            bav.mBGY = mBiasUpdate.mBGY;
            bav.mBGZ = mBiasUpdate.mBGZ;
            bav.mBAX = mBiasUpdate.mBAX;
            bav.mBAY = mBiasUpdate.mBAY;
            bav.mBAZ = mBiasUpdate.mBAZ;
            ...
            for (size_t i = 0; i < aux1.size(); i++)
                IntegrateNewMeasurement(aux1[i].mAcc, aux1[i].mGyr, aux1[i].mTs);
            for (size_t i = 0; i < aux2.size(); i++)
                IntegrateNewMeasurement(aux2[i].mAcc, aux2[i].mGyr, aux2[i].mTs);
        }
 ...
 */
        void Preintegrated::SetNewBias(const Bias &bu_) {
            ...
            mBiasDelta(0) = bu_.mBGX - mBiasOri.mBGX;
            mBiasDelta(1) = bu_.mBGY - mBiasOri.mBGY;
            mBiasDelta(2) = bu_.mBGZ - mBiasOri.mBGZ;
            mBiasDelta(3) = bu_.mBAX - mBiasOri.mBAX;
            mBiasDelta(4) = bu_.mBAY - mBiasOri.mBAY;
            mBiasDelta(5) = bu_.mBAZ - mBiasOri.mBAZ;
        }
 ...
 */
        IMU::Bias Preintegrated::GetDeltaBias(const Bias &b_) {
            std::unique_lock<std::mutex> lock(mMutex);
            return IMU::Bias(b_.mBAX - mBiasOri.mBAX, b_.mBAY - mBiasOri.mBAY, b_.mBAZ - mBiasOri.mBAZ, b_.mBGX - mBiasOri.mBGX, b_.mBGY - mBiasOri.mBGY,
                             b_.mBGZ - mBiasOri.mBGZ);
        }
 ...
 */
        Eigen::Matrix3f Preintegrated::GetDeltaRotation(const Bias &b_) {
            ...
            // 计算偏置的变化量
            Eigen::Vector3f dbg;
            dbg << b_.mBGX - mBiasOri.mBGX, b_.mBGY - mBiasOri.mBGY, b_.mBGZ - mBiasOri.mBGZ;
 ...
 */
        Eigen::Vector3f Preintegrated::GetDeltaVelocity(const Bias &b_) {
            ...
            dbg << b_.mBGX - mBiasOri.mBGX, b_.mBGY - mBiasOri.mBGY, b_.mBGZ - mBiasOri.mBGZ;
            dba << b_.mBAX - mBiasOri.mBAX, b_.mBAY - mBiasOri.mBAY, b_.mBAZ - mBiasOri.mBAZ;
 ...
 */
        Eigen::Vector3f Preintegrated::GetDeltaPosition(const Bias &b_) {
            std::unique_lock<std::mutex> lock(mMutex);
            Eigen::Vector3f DeBiasGyr, DeBiasAcc;
            DeBiasGyr << b_.mBGX - mBiasOri.mBGX, b_.mBGY - mBiasOri.mBGY, b_.mBGZ - mBiasOri.mBGZ;
            DeBiasAcc << b_.mBAX - mBiasOri.mBAX, b_.mBAY - mBiasOri.mBAY, b_.mBAZ - mBiasOri.mBAZ;
            // 考虑偏置后，dP对偏置线性化的近似求解,邱笑晨《预积分总结与公式推导》P13，JPg和JPa在预积分处理中更新
            return dP + JPg * DeBiasGyr + JPa * DeBiasAcc;
        }
 ...
 */
        Eigen::Matrix3f Preintegrated::GetUpdatedDeltaRotation() {
            std::unique_lock<std::mutex> lock(mMutex);
            return NormalizeRotation(dR * Sophus::SO3f::exp(JRg * mBiasDelta.head(3)).matrix());
        }
 ...
 */
        Eigen::Vector3f Preintegrated::GetUpdatedDeltaVelocity() {
            std::unique_lock<std::mutex> lock(mMutex);
            return dV + JVg * mBiasDelta.head(3) + JVa * mBiasDelta.tail(3);
        }
 ...
 */
        Eigen::Vector3f Preintegrated::GetUpdatedDeltaPosition() {
            std::unique_lock<std::mutex> lock(mMutex);
            return dP + JPg * mBiasDelta.head(3) + JPa * mBiasDelta.tail(3);
        }
 ...
 */
        void Calib::Set(const Sophus::SE3<float> &sophTbc, const float &ng, const float &na, const float &ngw,
            ...
            mTcb = mTbc.inverse();
            // 噪声协方差
            CovNoise.diagonal() << ng2, ng2, ng2, na2, na2, na2;
            // 随机游走协方差
